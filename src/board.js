...
Board.prototype.makeBoard = size => {
    let b = [];
...
Board.prototype.fillBoard = board => {
...
Board.prototype.canMoveChecker = function(checker, row, col) {
    let player = this.checkers[checker].player,
        moves = this.getAllMoves(player),
        movesToCheck = moves.jumps.length ? moves.jumps : moves.singles;
...
Board.prototype.moveChecker = function (checker, row, col) {
    ...
    if (this.isJumpMove(checker, row)) {
        let midRow = (cRow + row) / 2,
            midCol = (cCol + col) / 2,
            removedPlayer = this.board[midRow][midCol];
...
Board.prototype.makeCheckers = (size, playerOne, playerTwo) => {
    let checkers = [],
        num = (size/2) * 3,
        row = size-1,
        col = 0;
